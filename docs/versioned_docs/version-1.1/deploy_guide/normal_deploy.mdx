...
在 Dinky 根目录 sql 文件夹下分别放置了 datastudio-mysql.sql 、 upgrade/${version}_schema/mysql/ddl 和 dml。如果第一次部署，可以直接将 sql/datastudio-mysql.sql 文件在 dinky 数据库下执行。（如果之前已经部署，那 upgrade 目录下存放了各版本的升级 sql ，根据版本号按需执行即可）
...
mysql -h fdw1  -udinky -pdinky
mysql> use dinky;
mysql> source /opt/dinky/sql/datastudio-mysql.sql
```
...
```shell
tar -zxf datastudio-release-${flink.version}-${dinky.version}.tar.gz
mv datastudio-release-${flink.version}-${dinky.version} dinky
cd dinky
```
...
当然如果你的 Hadoop 为 3+ 也可以自行编译对于版本的 datastudio-client-hadoop.jar 以替代 uber 包，
...
│   │   └── dinky
│   │       ├── datastudio-catalog-mysql-1.14-${VERSION}.jar -- dinky 的 catalog 实现
│   │       ├── datastudio-client-1.14-${VERSION}.jar
│   │       └── datastudio-connector-jdbc-1.14-${VERSION}.jar -- dinky 的 jdbc 连接器增强实现
│   ├── flink1.15  -- 适配 Flink1.15.x
│   │   └── dinky
│   │       ├── datastudio-catalog-mysql-1.15-${VERSION}.jar -- dinky 的 catalog 实现
│   │       └── datastudio-client-1.15-${VERSION}.jar
│   ├── flink1.16  -- 适配 Flink1.16.x
│   │   └── dinky
│   │       ├── datastudio-catalog-mysql-1.16-${VERSION}.jar -- dinky 的 catalog 实现
│   │       └── datastudio-client-1.16-${VERSION}.jar
│   ├── flink1.17  -- 适配 Flink1.17.x
│   │   └── dinky
│   │       ├── datastudio-catalog-mysql-1.17-${VERSION}.jar -- dinky 的 catalog 实现
│   │       └── datastudio-client-1.17-${VERSION}.jar
│   └── flink1.18  -- 适配 Flink1.18.x
│   │   └── dinky
│   │       ├── datastudio-catalog-mysql-1.18-${VERSION}.jar -- dinky 的 catalog 实现
│   │       └── datastudio-client-1.18-${VERSION}.jar
├── jar
│   ├── datastudio-app-1.14-${VERSION}-jar-with-dependencies.jar -- 适配 Flink1.14.x application
│   ├── datastudio-app-1.15-${VERSION}-jar-with-dependencies.jar -- 适配 Flink1.15.x application
│   ├── datastudio-app-1.16-${VERSION}-jar-with-dependencies.jar -- 适配 Flink1.16.x application
│   ├── datastudio-app-1.17-${VERSION}-jar-with-dependencies.jar -- 适配 Flink1.17.x application
│   └── datastudio-app-1.18-${VERSION}-jar-with-dependencies.jar -- 适配 Flink1.18.x application
├── lib
│   └── ... -- Dinky 的内部依赖
└── sql
    ├── datastudio-mysql.sql -- Mysql 首次初始化脚本
    ├── datastudio-pg.sql -- PGSql 首次初始化脚本
    └── upgrade  -- 各个版本的升级 SQL
```
