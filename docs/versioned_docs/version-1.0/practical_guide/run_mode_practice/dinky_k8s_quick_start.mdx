...
- datastudio-app-1.15-1.0.0-SNAPSHOT-jar-with-dependencies.jar
...
```shell
# 构建Dinky app镜像
docker build -t datastudio-flink:1.0.0-1.15.4 . --no-cache
# 这一步为推送到私有镜像仓库过程，请根据需要自行修改参数
docker tag datastudio-flink:1.0.0-1.15.4 192.168.0.10:5000/datastudio-flink-1.0.0-1.15.4
docker push 192.168.0.10:5000/datastudio-flink-1.0.0-1.15.4
```
...
| Flink 镜像地址         | 上一步打包的镜像地址                               |  是   |  无  | datastudio-flink-1.0.0-1.15.4 |
...
| 参数       | 说明                                                         | 是否必填 | 默认值 |                                示例值                                |
|----------|------------------------------------------------------------|:----:|:---:|:-----------------------------------------------------------------:|
| Jar 文件路径 | 指定镜像内datastudio-app的 Jar 文件路径,如果该集群配置用于提交 Application 模式任务时 则必填 |  否   |  无  | local:///opt/flink/datastudio-app-1.16-1.0.0-jar-with-dependencies.jar |
